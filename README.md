...
> 当前状态：计划一至计划六及 F11 磁盘健康只读版的已评审范围已接入。应用支持只读空间扫描、版本化清理预览、四类缓存隔离与恢复、回收站官方 API、物理磁盘拓扑、分区合并可行性预演，以及 Windows 存储可靠性健康检测；Windows Update 管理员维护、隔离区永久删除和任何分区写操作均未开放。
...
当前版本只在固定允许根内执行可恢复隔离，并可通过独立确认调用 Windows 回收站官方 API。分区功能完全只读，预演结果始终不授权执行。磁盘健康检测只运行固定 Storage 查询，未知字段不会被推断为健康，序列号仅保留末四位。所有未来高权限操作都必须遵循 [特权操作安全规范](docs/security/privileged-operations.md)。
