...
> 状态：完整恢复与永久删除闭环已实现 · 安全级别：普通用户受限写入 · 更新时间：2026-08-17

## 1. 范围

当前闭环为“规则发现 → 用户选择 → 不可变计划 → 新扫描复核 → 模式绑定的一次性确认 → 执行开始审计 → 隔离/官方接口 → 结果审计 → 单项或批量恢复”。四个版本化缓存适配器可进入隔离区；回收站使用完全独立的官方 API 边界。

永久删除已作为独立不可恢复边界实现，但只能处理应用隔离索引中的后端 entry ID；回收站仍只调用 `SHEmptyRecycleBinW`。这两个边界互不复用令牌，均不接受路径、命令、Windows 服务名或分区参数。
...
- 原位置存在时返回 `RestoreConflict`，不覆盖。备用目标只接受 `Desktop/Documents/Downloads` 枚举，并恢复到后端解析的 `Clarity Disk Restored` 子目录。

永久删除先绑定 1–100 个 entry 当前元数据摘要、随机两分钟令牌和“确认永久删除隔离项目”。消费后再次读取索引并比较摘要；每项必须仍位于应用隔离根且整树无重解析点。成功状态为 `PermanentlyDeleted`，部分失败保留原状态。回收站永久清空不能扩展为任意文件删除能力。
...
审计事件覆盖扫描、计划、索引、确认、执行、原位置/备用位置恢复和永久删除开始/结果。事件只记录类型、关联 ID、规则 ID、数量、容量、时间和安全原因，不记录文件内容。
...
普通用户隔离已完成专用枚举协议、固定策略、同/跨卷事务、原位置/受控位置恢复、一次性永久删除和回收站官方接口。任何高权限系统清理仍必须使用独立受限管理员代理，不能复用普通用户隔离令牌。
