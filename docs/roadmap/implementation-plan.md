...
当前已完成：P0 基础架构、首页和 P1 系统盘容量的只读发现；GitHub CI 已验证通过。

下一次实现建议：

1. 把系统盘发现扩展为多磁盘只读列表和卷健康基础信息。
2. 在 `clarity-core` 增加扫描任务状态、取消语义和 `CleanupCandidate` 模型。
3. 接入一个低风险浏览器缓存规则，只生成预览，不执行删除。
4. 为扫描结果、路径边界和规则版本补齐 Rust 测试。
5. UI 增加扫描页和清理预览页，再评审后决定是否开放首个低风险清理动作。
