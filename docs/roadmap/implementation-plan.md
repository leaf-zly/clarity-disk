...
> 文档版本：0.6 · 更新时间：2026-08-14 · 状态：计划七分区安全基础已完成
...
| P7.1 分区写实现   | 数据迁移、卷调整、重启恢复持久化、回滚保护           | 高风险 | 通过独立安全评审后，以功能开关灰度发布                |
...
当前已完成：P0 基础架构、首页、P1 多磁盘/多卷只读发现、完整 P2 空间扫描、完整 P3 清理规则与预览、P4 的计划三至计划五、P6 计划六的只读物理分区拓扑与合并预演、F11 磁盘健康只读版，以及 P7 分区安全基础。Rust 已实现受限清理、恢复、回收站官方 API、失败关闭的分区预演、保守健康聚合、摘要绑定安全计划和恢复状态机；Vue 只提交后端 ID 和固定枚举，不提交执行路径或命令文本。

下一次实现建议：

1. 优先实现计划八的独立、最小权限、版本化管理员服务协议；普通用户隔离令牌不得复用。
2. 为计划 P7.1 增加独立备份提供程序、恢复日志持久化和 Windows 故障注入实验环境，真实写入继续关闭。
