...
这份文档用于统一产品范围、技术路线和安全边界。当前仓库已经完成 Rust workspace、Tauri + Vue 3 首页、GitHub CI/CD、Windows 系统盘只读发现，以及计划四/五的隔离区产品化和版本化清理适配器；隔离区永久删除、管理员系统维护和分区写操作仍未开放。
...
当前已完成：P0 基础架构、首页、P1 多磁盘/多卷只读发现、完整 P2 空间扫描、完整 P3 清理规则与预览，以及 P4 的计划三至计划五。Rust 已实现四类用户缓存隔离、同卷原子移动、跨卷复制校验事务、启动保守恢复、7/15/30 天保留策略、1/5/10/20 GiB 容量档位、到期只标记、最多 100 项批量恢复，以及回收站官方 Shell API 独立永久确认；Vue 只提交计划/候选/条目 ID、固定策略档位和后端令牌，不提交路径。

下一次实现建议：

1. 隔离区永久删除保持关闭，完成独立威胁建模、擦除边界与故障注入安全评审后再规划开放。
