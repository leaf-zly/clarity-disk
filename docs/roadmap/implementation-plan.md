...
> 文档版本：1.0 · 更新时间：2026-08-17 · 状态：计划十至十三工程实现完成，危险能力按部署证据门禁

这份文档用于统一产品范围、技术路线和安全边界。当前仓库已完成 P0–P8 的工程范围，以及计划十的独立备份证据/可销毁 VHDX 实验环境、计划十一的历史/设置/自动维护、计划十二的更新/诊断/签名发布强化、计划十三的隔离永久删除与受控目标恢复。真实分区写操作仍要求有效外部备份凭据、专用实验记录、签名证书和双重功能门禁；默认安装不会绕过这些部署证据。
