...
> 版本：1.0 · 适用范围：计划七安全基础 · 当前写入能力：未实现

## 1. 安全结论

计划七只建立未来分区写操作所需的安全协议，不执行磁盘修改。当前 Tauri 命令面只允许使用后端发现的源/目标分区 ID 生成只读预演和不可变安全计划；`execution_authorized` 与 `write_capability_present` 固定为 `false`。
...
当前版本不存在从 `PLAN` 到 `PRIV` 的可调用边，也不存在 `PRIV` 进程。
...
- 最小权限特权服务和版本化 IPC 协议；
- 一次性确认令牌、计划持久化和审计持久化；
- 真实数据迁移、校验、卷缩放和故障注入实验环境；
- 安全评审、代码签名、灰度开关与恢复演练。

在这些门槛全部完成前，分区写操作必须保持不可见且不可调用。
