...
> 当前状态：计划一至计划六的已评审范围已接入。应用支持只读空间扫描、版本化清理预览、四类缓存隔离与恢复、回收站官方 API，以及物理磁盘拓扑和分区合并可行性预演；Windows Update 管理员维护、隔离区永久删除和任何分区写操作均未开放。
...
当前版本只在固定允许根内执行可恢复隔离，并可通过独立确认调用 Windows 回收站官方 API。分区功能完全只读，预演结果始终不授权执行。所有未来高权限操作都必须遵循 [特权操作安全规范](docs/security/privileged-operations.md)。
