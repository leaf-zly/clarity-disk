...
受控执行使用独立二进制 `clarity-privileged-service`。它不是常驻 Windows 服务，而是一次 UAC 对应一次进程的一次性管理员代理：请求完成后立即退出，减少高权限常驻攻击面。普通 Tauri 进程继续以当前用户运行。

该代理实现相邻普通 NTFS 数据分区的真实执行链：重新发现 → 校验身份和证据 → 迁移与 SHA-256 校验 → 持久化恢复检查点 → 删除右侧源分区 → 扩展左侧目标分区 → 后置校验。实现使用固定 PowerShell Storage cmdlet 与受信任的 `robocopy.exe`，协议中没有脚本、命令或路径字段。
