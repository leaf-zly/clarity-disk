...
> 文档版本：0.5 · 更新时间：2026-08-14 · 状态：F11 磁盘健康只读版已完成

这份文档用于统一产品范围、技术路线和安全边界。当前仓库已经完成 Rust workspace、Tauri + Vue 3 首页、GitHub CI/CD、Windows 系统盘只读发现、计划四/五的隔离区产品化和版本化清理适配器、计划六的物理分区拓扑与合并可行性预演，以及 F11 磁盘健康只读检测；隔离区永久删除、管理员系统维护和分区写操作仍未开放。
...
当前已完成：P0 基础架构、首页、P1 多磁盘/多卷只读发现、完整 P2 空间扫描、完整 P3 清理规则与预览、P4 的计划三至计划五、P6 计划六的只读物理分区拓扑与合并预演，以及 F11 磁盘健康只读版。Rust 已实现四类用户缓存隔离、同卷原子移动、跨卷复制校验事务、启动保守恢复、固定隔离策略、批量恢复、回收站官方 API、失败关闭的分区预演规则和保守健康聚合；Vue 只提交后端 ID 和固定枚举，不提交执行路径或命令文本。

下一次实现建议：

1. 计划七分区写操作继续保持关闭，先完成独立威胁建模、备份/供电前置条件、恢复状态机和故障注入评审。
2. 为 Windows Update 设计独立、最小权限、受限管理员服务；普通用户隔离令牌不得复用。
