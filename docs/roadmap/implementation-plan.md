...
> 文档版本：0.7 · 更新时间：2026-08-14 · 状态：计划八/九工程实现完成，分区上线门禁关闭

这份文档用于统一产品范围、技术路线和安全边界。当前仓库已经完成 Rust workspace、Tauri + Vue 3 首页、GitHub CI/CD、Windows 系统盘只读发现、计划四/五的隔离区产品化、计划六的物理分区预演、F11 磁盘健康只读检测，以及计划七的分区不可变安全计划与恢复协议；隔离区永久删除、管理员系统维护和真实分区写操作仍未开放。
...
| P4 低风险清理     | 隔离区/回收站、官方 Windows 清理接口、一次性确认令牌 | 受限   | 计划四/五已完成；四类缓存可隔离，回收站使用 Shell API |
| P5 高级系统维护   | 休眠文件、更新缓存、还原点等需确认的系统项           | 受限   | 计划八已完成；独立一次性管理员代理与三类适配器        |
...
| P7.1 分区写实现   | 数据迁移、卷调整、恢复持久化、回滚保护               | 高风险 | 计划九工程实现完成；备份/演练/签名上线门禁仍关闭      |
...
- `execution_authorized` 固定为 `false`，计划六不存在分区写操作、确认令牌、管理员服务、数据迁移或重启任务。

### 5.2 F11 磁盘健康只读实现结果

- `clarity-core::health` 提供物理磁盘身份、提供程序/自监测状态、证据完整性、温度、磨损、错误计数、加密汇总和保守信号聚合。
- Windows 适配器执行无调用方参数的固定 Storage 查询，按 `UniqueId`、磁盘号优先级映射计划六身份；完整序列号在 JSON 输出前截断。
...
- 当前没有独立备份提供程序，因此用户勾选不会伪装成备份验证，真实 Windows 评估保持备份阻塞。
...
当前已完成计划八/九的工程实现：独立一次性管理员代理、版本化无路径协议、休眠/Windows Update 下载缓存/系统还原点适配器、一次性 UAC 握手、管理员审计、计划 v2 全身份绑定、真实数据迁移与摘要校验、恢复日志、固定删除/扩展适配器和后置验证。Vue 只提交后端 ID 和固定枚举，不提交执行路径或命令文本。

下一次实现建议：

1. 为计划 P7.1 增加独立备份提供程序和 Windows 可销毁磁盘故障注入实验环境；真实写入继续关闭。
2. 完成管理员代理和桌面程序 Authenticode 签名、UAC 发布者验证及安全评审。
3. 厂商专有原始 SMART 属性保持兼容性研究项，不用缺失字段替代 Windows 统一可靠性结论。
4. 隔离区永久删除保持关闭，完成擦除边界与恢复策略安全评审后再单独规划。
